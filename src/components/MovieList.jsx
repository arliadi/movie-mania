  ...
  return (
    <>
      {displayData.map((movie) => (
        <a href={`/detail/${movie.id}`} key={movie.id} onClick={(e) => {setSelectedMovie(movie.id); handleOpenModal(e)}}>
            <div className="card" style={{ width: '15rem' }}>
              <img className="Movie-image" src={`${baseImgUrl}${movie.poster_path}`} />
              <div className="card-body">
                <div className="Movie-title">{movie.title || ""}</div>
                <div className="Movie-date">Release: {movie.release_date ? formatTanggal(movie.release_date) : ""}</div>
              ...
              {selectedMovieResult && (
                <div className="row">
                  ...
                  <div className="col-md-8">
                    <h6>Release: {formatTanggal(selectedMovieResult.release_date)}</h6>
                    <p>Rating: ⭐ {Number(selectedMovieResult.vote_average).toFixed(1)}</p>
